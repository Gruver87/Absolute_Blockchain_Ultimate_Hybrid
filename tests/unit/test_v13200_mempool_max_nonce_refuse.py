#!/usr/bin/env python3
"""v1.3.200: P2P refuses oversized nonce before validate_transaction."""
...
def _node() -> P2PNode:
    ...
    cfg.bootstrap_peers = []
    cfg.p2p_mempool_max_nonce_refuse = True
    cfg.p2p_mempool_max_nonce = 1_000_000_000_000
...
def test_needles_v13200():
    p2p = (ROOT / "network" / "p2p_node.py").read_text(encoding="utf-8")
    assert "nonce_too_high" in p2p
    assert "p2p_mempool_max_nonce_refuse" in p2p
    assert "native_mempool_max_nonce_refuse" in p2p
    assert "_mempool_nonce_high_refuse_total" in p2p
    cfg = (ROOT / "runtime" / "config.py").read_text(encoding="utf-8")
    assert "p2p_mempool_max_nonce_refuse" in cfg
    assert "p2p_mempool_max_nonce" in cfg
    assert "P2P_MEMPOOL_MAX_NONCE_REFUSE" in cfg
    notes = (ROOT / "RELEASE_NOTES_v1.3.200.md").read_text(encoding="utf-8")
    assert "1.3.200-industrial" in notes
    assert Config().node_version.startswith("1.3.200")
    metrics = (ROOT / "observability" / "metrics.py").read_text(encoding="utf-8")
    assert "abs_p2p_native_mempool_max_nonce_refuse" in metrics
    assert "abs_p2p_mempool_nonce_high_refuse_total" in metrics


def test_refuse_oversized_nonce_before_validate():
    node = _node()
    out = _build(node, _wire(nonce=1_000_000_000_001))
    assert out is None
    assert node._last_tx_wire_reject == "nonce_too_high"
    assert node._mempool_nonce_high_refuse_total >= 1
    node.blockchain.validate_transaction.assert_not_called()


def test_ok_max_nonce_reaches_validate():
    node = _node()
    out = _build(node, _wire(nonce=1_000_000_000_000, tx_hash="cd" * 32))
    assert out is not None
    node.blockchain.validate_transaction.assert_called()
    assert node._mempool_nonce_high_refuse_total == 0


def test_security_status_gauge():
    node = _node()
    st = node.get_p2p_security_status()
    assert st.get("native_mempool_max_nonce_refuse") is True

...
| **Industrial tip-v2 re-smoke** | **PASS** 2026-08-02 — fresh volume wipe + tip-v2 arm; ready×3 + probe + failover + signed tx + EVM; DR + ceremony pin + secrets dry-run earlier in session; **48h soak RUNNING** on clean tip-v2 volumes (`logs/industrial_tipv2_soak_48h.log` → `logs/soak_report_tipv2_48h.json`) — claim soak only when `passed=true` | `docs/evidence/runs/8c92a51f0144/` + [INDUSTRIAL_HARDEN_RUNBOOK.md](INDUSTRIAL_HARDEN_RUNBOOK.md) |

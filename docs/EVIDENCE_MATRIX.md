...
| Storage canonical cutover (`Blockchain` + `open_storage`, ADR 0006 D–E) | **Integration-wired** — `add_block` persist via StoragePort UoW (join open atomic); factory + main DI; cutover tests (CAS/ENOSPC/import/tip agree); execution still on legacy `self.db`; **not** full `self.db` purge / live disk-fill |

...
**Recent code hardening (v1.3.03–v1.3.14, not organizational proof):** P2P shape/rate-limit/ops_errors/peer_tx_reject Prometheus metrics + alerts; sync `state_consistent` / wire-probe gauges with fail-closed empty/timeout probes and False defaults; ceremony dir auto-detect; RocksDB tuning env/JSON parity + reorg/get_stats fail-loud; **SQLite reorg parity** (evm_logs/tx_prop + full truncate); L1 RPC unprobed ≠ green; wire-parse/rate-limit/housekeeping/mid-session-handshake/semantic-tx rejects strike with pre-ban warning logs; CI `final_audit` blocking; honest send/status/import/sync/discovery fail counters; prod `/health/ready` requires `p2p_running`; k8s ConfigMap embed freeze + post_soak k8s gate + bridge-OFF freeze; mesh Redis JSON honesty; compose↔JSON freeze (3-node + single-node) + p2ptls FAIL_CLOSED/BIND_IDENTITY overlays; CORS RPC proxy no hardcoded `*`; EVM CREATE native fail-closed; `.env.example` bridge/CORS defaults OFF + mainnet chain_id note.

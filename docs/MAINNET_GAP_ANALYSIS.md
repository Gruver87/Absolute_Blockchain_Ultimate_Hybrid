...
**Recent code hardening (v1.3.03–v1.3.11, not organizational proof):** P2P shape/rate-limit/ops_errors/peer_tx_reject Prometheus metrics + alerts; sync `state_consistent` / wire-probe gauges; ceremony dir auto-detect; RocksDB tuning env/JSON parity (CF remains **operator opt-in**, default false); wire-parse/rate-limit/housekeeping/mid-session-handshake/semantic-tx rejects strike with pre-ban warning logs; CI `final_audit` blocking; honest send/status/import/sync/discovery fail counters; k8s ConfigMap embed freeze + post_soak k8s gate + bridge-OFF freeze; mesh Redis JSON honesty; compose↔JSON numeric freeze (incl. bridge/redis defaults); EVM CREATE native fail-closed; `.env.example` bridge default OFF.
